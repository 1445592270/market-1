...
class WeixinPay(object):
    ...
    def sign(self, raw):
        raw = [
            (k, str(raw[k]) if isinstance(raw[k], int) else raw[k])
    ...
    def to_dict(self, content):
        ...
        root = etree.fromstring(
            content.encode("utf-8"), parser=etree.XMLParser(resolve_entities=False)
        )
        for child in root:
    ...
    def _fetch(self, url, data, use_cert=False, appid=True):
        ...
        if use_cert:
            resp = self.sess.post(
                url, data=self.to_xml(data), cert=(self.cert, self.key)
            )
        else:
    ...
    def unified_order(self, **data):
        """
        统一下单
        out_trade_no、body、total_fee、trade_type 必填
        app_id, mchid, nonce_str 自动填写
        spbill_create_ip 在 flask 框架下可以自动填写，非 flask 框架需要主动传入此参数

        body:
            浏览器打开的网站主页 title 名 - 商品概述
            腾讯充值中心 -QQ 会员充值
        """
        ...
        if "out_trade_no" not in data:
            raise WeixinPayError("缺少统一支付接口必填参数 out_trade_no")
        if "body" not in data:
            raise WeixinPayError("缺少统一支付接口必填参数 body")
        if "total_fee" not in data:
            raise WeixinPayError("缺少统一支付接口必填参数 total_fee")
        if "trade_type" not in data:
            raise WeixinPayError("缺少统一支付接口必填参数 trade_type")

        # 关联参数
        if data["trade_type"] == "JSAPI" and "openid" not in data:
            raise WeixinPayError("trade_type 为 JSAPI 时，openid 为必填参数")
        if data["trade_type"] == "NATIVE" and "product_id" not in data:
            raise WeixinPayError("trade_type 为 NATIVE 时，product_id 为必填参数")
        data.setdefault("notify_url", self.notify_url)
        if "spbill_create_ip" not in data:
            raise WeixinPayError("spbill_create_ip 为必填参数")
    ...
    def refund(self, **data):
        """
        申请退款
        out_trade_no、transaction_id 至少填一个且
        ...
        if "out_trade_no" not in data and "transaction_id" not in data:
            raise WeixinPayError("退款申请接口中，out_trade_no、transaction_id 至少填一个")
        if "out_refund_no" not in data:
            raise WeixinPayError("退款申请接口中，缺少必填参数 out_refund_no")
        if "total_fee" not in data:
            raise WeixinPayError("退款申请接口中，缺少必填参数 total_fee")
        if "refund_fee" not in data:
            raise WeixinPayError("退款申请接口中，缺少必填参数 refund_fee")
    ...
    def refund_query(self, **data):
        ...
        用零钱支付的退款 20 分钟内到账，银行卡支付的退款 3 个工作日后重新查询退款状态。
        out_refund_no、out_trade_no、transaction_id、refund_id 四个参数必填一个
        appid、mchid、nonce_str 不需要填入
        """
        url = self.PAY_HOST + "/pay/refundquery"
        if (
            "out_refund_no" not in data
            and "out_trade_no" not in data
            and "transaction_id" not in data
            and "refund_id" not in data
        ):
            raise WeixinPayError(
                "退款查询接口中，out_refund_no、out_trade_no、transaction_id、refund_id 四个参数必填一个"
            )
    ...
    def download_bill(self, bill_date, bill_type="ALL", **data):
        ...
        if "bill_date" not in data:
            raise WeixinPayError("对账单接口中，缺少必填参数 bill_date")
    ...
    def pay_individual(self, **data):
        ...
        if "partner_trade_no" not in data:
            raise WeixinPayError("企业付款接口中，缺少必要的参数 partner_trade_no")
    ...
    def pay_individual_to_card(self, **data):
        ...
        if "enc_bank_no" not in data:
            raise WeixinPayError("企业付款接口中，缺少必填参数 enc_bank_no")
        if "enc_true_name" not in data:
            raise WeixinPayError("企业付款接口中，缺少必填参数 enc_true_name")
        if "bank_code" not in data:
            raise WeixinPayError("企业付款接口中，缺少必填参数 bank_code")
        if "amount" not in data:
            raise WeixinPayError("企业付款接口中，缺少必填参数 amount")
