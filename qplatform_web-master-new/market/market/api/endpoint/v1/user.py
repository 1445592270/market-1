...
from market.models import MarketUser, StrategyMarket
...
async def register_user(user_in: UserCreate, response: Response):
    ...
    row = await MarketUser.select("id").where(MarketUser.name == user_in.name).gino.all()
    if row:
        raise HTTPException(
            status_code=403,
    ...
    if user_in.phone:
        row = await MarketUser.select("id").where(MarketUser.phone == user_in.phone).gino.all()
        if row:
            raise HTTPException(
                status_code=403,
    ...
    if user_in.email:
        row = await MarketUser.select("id").where(MarketUser.email == user_in.email).gino.all()
        if row:
            raise HTTPException(
                status_code=403,
...
async def recover_password(schema_in: ResetPassword):
    ...
    if schema_in.phone:
        user = await MarketUser.query.where(MarketUser.phone==schema_in.phone).gino.first()
        #password = await MarketUser.select('password').where(MarketUser.phone==schema_in.phone).gino.scalar()
    else:
        user = await MarketUser.query.where(MarketUser.email==schema_in.email).gino.first()
    ...
    await user.update(password=get_password_hash(schema_in.password, user.uuid.hex)).apply()
    #await user.save()
    # verify_code = "1234"
    # if schema_in.phone:
